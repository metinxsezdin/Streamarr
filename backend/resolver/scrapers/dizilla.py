"""
Dizilla scraper implementation.
"""
            ...
            (() => {
                ...
                const selectors = [
                    ...
                    elements.slice(0, 5).forEach(element => {
                        const key = element.outerHTML || element.textContent || selector;
                        if (visited.has(key)) {
                            return;
                        }
                        visited.add(key);
...
def main() -> None:
    default_url = "https://dizilla40.com/murdaugh-death-in-the-family-1-sezon-1-bolum"
    args = sys.argv[1:]
